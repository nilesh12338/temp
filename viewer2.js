...
function () {
  ...
  _createClass(PDFPageView, [{
    ...
    value: function paintOnCanvas(canvasWrapper) {
      ...
      // Ensure canvas pixel dimensions do not exceed the visible wrapper (viewport)
      var fullScaleFactor = 1;
      try {
        var wrapperWidth = canvasWrapper && canvasWrapper.clientWidth ? canvasWrapper.clientWidth : window.innerWidth || 1024;
        var wrapperHeight = canvasWrapper && canvasWrapper.clientHeight ? canvasWrapper.clientHeight : window.innerHeight || 768;
        var maxPixelWidth = Math.max(1, Math.floor(wrapperWidth * outputScale.sx));
        var maxPixelHeight = Math.max(1, Math.floor(wrapperHeight * outputScale.sy));

        if (canvas.width > maxPixelWidth || canvas.height > maxPixelHeight) {
          fullScaleFactor = Math.min(maxPixelWidth / canvas.width, maxPixelHeight / canvas.height);
          fullScaleFactor = Math.max(0.01, fullScaleFactor);
          canvas.width = Math.max(1, Math.floor(canvas.width * fullScaleFactor));
          canvas.height = Math.max(1, Math.floor(canvas.height * fullScaleFactor));
          // Keep CSS size as the intended viewport CSS dimensions so the
          // visual size doesn't change; the drawing scale is adjusted below.
          canvas.style.width = (0, _ui_utils.roundToDivide)(viewport.width, sfx[1]) + 'px';
          canvas.style.height = (0, _ui_utils.roundToDivide)(viewport.height, sfy[1]) + 'px';
        }
      } catch (e) {
      ...
      if (outputScale.scaled) {
        var transformScaleX = outputScale.sx * fullScaleFactor;
        var transformScaleY = outputScale.sy * fullScaleFactor;
        transform = [transformScaleX, 0, 0, transformScaleY, 0, 0];
      }
      var renderContext = {
        canvasContext: ctx,
        transform: transform,
